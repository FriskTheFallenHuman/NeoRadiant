...
xgettext_options = (
    " --c++ --keyword=_ --keyword=N_ -o "
    + pot_file
    + " --msgid-bugs-address=greebo@angua.at "
)
...
# Add menu captions
processXmlFile(
    os.path.join(build_root, xml_directory, "menu.xml"), ".//*", empty, ["caption"]
)

# Add colour scheme names
processXmlFile(
    os.path.join(build_root, xml_directory, "colours.xml"),
    ".//descriptions/*",
    empty,
    ["value"],
)
...
processXmlFile(
    os.path.join(build_root, xml_directory, "user.xml"),
    ".//toolbutton",
    empty,
    ["tooltip"],
)
...
for current_file in glob.glob(os.path.join(build_root, xml_directory, "ui", "*.xrc")):
    processXmlFile(
        current_file, ".//*", ["{http://www.wxwindows.org/wxxrc}label"], empty
    )
