...
print(
    "bc_rat can be modified"
    if GlobalMaterialManager.materialCanBeModified("bc_rat")
    else "bc_rat cannot be modified"
)
...
print(
    ...
    if GlobalMaterialManager.materialCanBeModified(bcRatCopy.getName())
    else "The copy cannot be modified?"
)
...
print(
    "The new material can be modified"
    if GlobalMaterialManager.materialCanBeModified(newMaterial.getName())
    else "The new material cannot be modified?"
)

renameResult = GlobalMaterialManager.renameMaterial(
    newMaterial.getName(), newMaterial.getName() + "_renamed"
)
print(
    "The rename operation was successful"
    if renameResult
    else "The rename operation failed"
)

# Do something with the new material
print(
    "The existing material reference now has the name {0}".format(newMaterial.getName())
)
...
print(
    "DecalInfo.stayMilliSeconds: {0}".format(
        newMaterial.getDecalInfo().stayMilliSeconds
    )
)
print(
    "DecalInfo.fadeMilliSeconds: {0}".format(
        newMaterial.getDecalInfo().fadeMilliSeconds
    )
)
...
newMaterial.setFrobStageRgbParameter(
    1, dr.Vector3(0.1, 0.2, 0.3)
)  # assign RGB components separately
...
editableDiffuseStage.addTransformation(
    dr.MaterialStage.TransformType.SCALE, "time", "global0"
)
rotateIndex = editableDiffuseStage.addTransformation(
    dr.MaterialStage.TransformType.ROTATE, "time*0.5", ""
)
editableDiffuseStage.updateTransformation(
    rotateIndex, dr.MaterialStage.TransformType.SCALE, "time*0.5", "0.5"
)
editableDiffuseStage.removeTransformation(rotateIndex)
editableDiffuseStage.setColourExpressionFromString(
    dr.MaterialStage.ColourComponent.RGB, "0.4*time"
)
...
for stage in newMaterial.getAllStages():
    ...
    print(
        "Stage blend func strings: {0},{1}".format(
            stage.getBlendFuncStrings()[0], stage.getBlendFuncStrings()[1]
        )
    )
    print(
        "Stage colour expression RED: {0}".format(
            stage.getColourExpressionString(dr.MaterialStage.ColourComponent.RED)
        )
    )
    print(
        "Stage colour expression GREEN: {0}".format(
            stage.getColourExpressionString(dr.MaterialStage.ColourComponent.GREEN)
        )
    )
    print(
        "Stage colour expression BLUE: {0}".format(
            stage.getColourExpressionString(dr.MaterialStage.ColourComponent.BLUE)
        )
    )
    print(
        "Stage colour expression ALPHA: {0}".format(
            stage.getColourExpressionString(dr.MaterialStage.ColourComponent.ALPHA)
        )
    )
    print(
        "Stage colour expression RGB: {0}".format(
            stage.getColourExpressionString(dr.MaterialStage.ColourComponent.RGB)
        )
    )
    print(
        "Stage colour expression RGBA: {0}".format(
            stage.getColourExpressionString(dr.MaterialStage.ColourComponent.RGBA)
        )
    )
    ...
    print(
        "Stage alpha test expression: {0}".format(stage.getAlphaTestExpressionString())
    )
    print(
        "Stage condition expression: {0}".format(stage.getConditionExpressionString())
    )
    print("Stage vertex program: {0}".format(stage.getVertexProgram()))
    print("Stage fragment program: {0}".format(stage.getFragmentProgram()))
    print(
        "Stage has {0} vertex parameters and {1} fragment maps".format(
    ...
    for v in range(0, stage.getNumVertexParms()):
        ...
        print(
            "Stage Vertex Parm #{0}: {1}".format(parm.index, " ".join(parm.expressions))
        )

    for f in range(0, stage.getNumFragmentMaps()):
        fm = stage.getFragmentMap(f)
        print(
            "Stage Fragment Map #{0}: {1} {2}".format(
                fm.index, " ".join(fm.options), fm.mapExpression
            )
        )
...
print(
    "The new material has been modified"
    if newMaterial.isModified()
    else "The new material is not modified?"
)
...
print(
    "After save, the material is not modified"
    if not newMaterial.isModified()
    else "After save, the material is still modified?"
)
