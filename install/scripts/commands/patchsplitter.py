...
def execute():
    ...
    class TooManyVerts(Exception):
        pass

    class TooFewVerts(Exception):
        pass
    ...
    class Vert:
        ...
        def __init__(self, vertex, texcoord):
            from neoradiant import Vector2, Vector3
            ...
            if patch:
                for rownum in range(patch.getHeight()):
                    ...
                    for colnum in range(patch.getWidth()):
                        v = Vert(
                            patch.ctrlAt(rownum, colnum).vertex,
                            patch.ctrlAt(rownum, colnum).texcoord,
                        )
                        row.append(v)
    ...
    def getAndValidateSelection():
        ...
        if (
            sInfo.patchCount != 1
            or sInfo.brushCount > 0
            or sInfo.entityCount > 0
            or sInfo.componentCount < 2
        ):
            raise UserError(
                "Bad selection. Select one patch only, and "
        ...
        if not patch.isValid():
            raise UserError(
                "This isn't a valid patch. It has some invalid "
    ...
    def distToEdge(patch, direction, axis, gridPower):
        ...
        looplimit = (
            extent() * 2
        ) / stepsize  # Safety valve. Don't let verts move further than the
        # extent of the patch. Stops an infinite loop crashing
        # DR if something goes wrong.
        stepcount = 0
        while (
            eval("boundary() %s starting_boundary" % comparison)
            and stepcount < looplimit
        ):
    ...
    def attemptGetVertsLine(patch, tolerance):
        ...
        minX = (
            AABB.origin.x()
            - AABB.extents.x()
            + distToEdge(patch, "left", "x", gridPower)
            - tolerance
        )
        maxX = (
            AABB.origin.x()
            + AABB.extents.x()
            - distToEdge(patch, "right", "x", gridPower)
            + tolerance
        )
        minZ = (
            AABB.origin.z()
            - AABB.extents.z()
            + distToEdge(patch, "down", "z", gridPower)
            - tolerance
        )
        maxZ = (
            AABB.origin.z()
            + AABB.extents.z()
            - distToEdge(patch, "up", "z", gridPower)
            + tolerance
        )
        ...
        minY = (
            AABB.origin.y()
            - AABB.extents.y()
            + distToEdge(patch, "left", "y", gridPower)
            - tolerance
        )
        maxY = (
            AABB.origin.y()
            + AABB.extents.y()
            - distToEdge(patch, "right", "y", gridPower)
            + tolerance
        )
        ...
        for row in range(patch.getHeight()):
            for col in range(patch.getWidth()):
                ...
                if (
                    minX <= vert.vertex.x() <= maxX
                    and minY <= vert.vertex.y() <= maxY
                    and minZ <= vert.vertex.z() <= maxZ
                ):
        ...
        if includedRows == set((0, patch.getHeight() - 1)) or includedCols == set(
            (0, patch.getWidth() - 1)
        ):
            raise UserError(
                "You've selected the existing seam of a 3d patch. It's already cut "
    ...
    def getSelectedVertsLine(patch, patchdata):
        ...
        while (not lineType) and mintolerance <= tolerance <= maxtolerance:
            try:
                print(
                    "Patch Splitter: Trying to identify selected verts with tolerance %0.2f"
                    % tolerance
                )
                lineType, lineNum = attemptGetVertsLine(
                    patch, tolerance
                )  # has side effects that can't be fixed till search \
            except (
                TooManyVerts
            ):  # \ is finished, as the fix clears the user's selection
                tolerance /= 2.0
            except TooFewVerts:
                tolerance *= 2
        ...
        if not lineType:
            raise UserError(
                "Unable to determine selected verts. Try again with different "
        ...
        if (
            ...
            raise UserError(
                "You've selected the existing edge of the patch. No action has been taken."
            )
        if lineNum % 2:
            raise UserError(
                "You've selected a green line. Patches can be cut only along lines with some pink verts."
            )
...
if __executeCommand__:
    try:
        ...
        GlobalDialogManager.createMessageBox(
            "Patch Splitter", str(e), dr.Dialog.ERROR
        ).run()
