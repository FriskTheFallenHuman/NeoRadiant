...
def execute():
    ...
    if not g_missing:
        ...
        GlobalDialogManager.createMessageBox(
            "Missing targets", msg, dr.Dialog.CONFIRM
        ).run()
    else:
        msg += "Missing targets:\n"
        ...
        response = GlobalDialogManager.createMessageBox(
            "Missing targets", msg, dr.Dialog.ASK
        ).run()
        if response == dr.Dialog.YES:
            ...
            class Selector(dr.SceneNodeVisitor):
                def pre(self, node):
                    if (
                        node.isEntity()
                        and node.getEntity().getKeyValue("name") in g_missing.keys()
                    ):
