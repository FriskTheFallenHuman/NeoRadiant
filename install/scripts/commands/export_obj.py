...
def execute():
    script = "NeoRadiant Wavefront OBJ Export (*.obj)"
    author = "Python port by greebo, based on original exporter C++ code in NeoRadiant and the ASE exporter scripts"
    version = "0.2"
    ...
    class Geometries(object):
        vertexCount = (
            0  # Global Vertex Index (every vertex in an OBJ file has a unique number)
        )
    ...
    def processBrush(brushnode):
        ...
        for index in range(numfaces):
            ...
            if (shader == "textures/common/caulk") and (
                int(GlobalRegistry.get("user/scripts/objExport/exportcaulk"))
            ) == 0:
                continue
            ...
            geometry.faces.append(
                [i for i in reversed(range(firstVertex + 1, geomlist.vertexCount + 1))]
            )

        print(
            "Processed brush geometry: {0} verts and {1} faces".format(
    ...
    def processPatch(patchnode):
        ...
        if (
            ...
            and int(GlobalRegistry.get("user/scripts/objExport/exportcaulk")) == 0
        ):
            return
        ...
        print(
            "Processed patch geometry: {0} verts and {1} faces".format(
                len(geometry.vertices), len(geometry.faces)
            )
        )
        geomlist.objects.append(geometry)
    ...
    dialog.setElementValue(
        fileHandle, GlobalRegistry.get("user/scripts/objExport/recentFilename")
    )
    ...
    dialog.setElementValue(
        centerObjectsHandle, GlobalRegistry.get("user/scripts/objExport/centerObjects")
    )

    # Add another checkbox
    exportCaulkHandle = dialog.addCheckbox("Export caulked faces")
    dialog.setElementValue(
        exportCaulkHandle, GlobalRegistry.get("user/scripts/objExport/exportcaulk")
    )
    ...
    if dialog.run() == dr.Dialog.OK:
        ...
        GlobalRegistry.set(
            "user/scripts/objExport/recentFilename", dialog.getElementValue(fileHandle)
        )
        GlobalRegistry.set(
            "user/scripts/objExport/recentPath", dialog.getElementValue(pathHandle)
        )
        GlobalRegistry.set(
            "user/scripts/objExport/centerObjects",
            dialog.getElementValue(centerObjectsHandle),
        )
        GlobalRegistry.set(
            "user/scripts/objExport/exportcaulk",
