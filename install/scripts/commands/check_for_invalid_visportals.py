...
def execute():
            ...
            if node.isBrush():
                ...
                if brush.hasShader(visportalShader):
                    ...
                    if brush.getParent().isEntity():
                        if (
                            brush.getParent().getEntity().getKeyValue("classname")
                            != "worldspawn"
                        ):
    ...
    msg = (
        "%d visportals checked, %d have errors"
        % (results.numVisportals, len(results.invalidPortals))
        + "\n\n"
    )
    ...
    GlobalDialogManager.createMessageBox(
        "Visportal Test Results", msg, dr.Dialog.CONFIRM
    ).run()
