...
While NeoRadiant focuses primarily on The Dark Mod, NeoRadiant is tailored for
...
NeoRadiant defaults to a black-on-white colour scheme in the 2D windows, but ships with
...
the system used by NeoRadiant may seem somewhat back-to-front. In previous
games, the world starts out as an infinite solid, in which you "carve out" rooms
using subtractive brushes. In NeoRadiant, the world starts out as an infinite
...
2D view and choose *Create player start here*. NeoRadiant will create the
...
Since a fully developed Dark Mod map is large and complex, NeoRadiant provides
