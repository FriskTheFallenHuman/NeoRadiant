...
NeoRadiant is a fork of DarkRadiant, a level editor for idTech4-based games.
While DarkRadiant focuses primarily on The Dark Mod, NeoRadiant is tailored for
editing generic maps targeting Idtech4 game engines.

=== Editing workflow

image::NeoRadiantWorkflow.png[align="center"]
...
installed into a directory tree which is accessible both by NeoRadiant (for
editing) and the game engine itself (for playing the mission).

Once assets have been arranged in NeoRadiant according to the wishes of the
...
realtime, making use of the same assets that were accessed by NeoRadiant during
editing.

NeoRadiant also maintains its own separate file containing various editing
...
accompanying `.project` file can still be edited with NeoRadiant, although
certain NeoRadiant-specific state may be lost.
...
which acts as the parent for new brushes and patches created in NeoRadiant.
...
When running NeoRadiant for the first time, the *Game Setup* dialog will be
shown. This is where you configure the path to the game installation from which
NeoRadiant will load assets, such as textures, models and entity definitions.

image::GameSetupDialog.png[align="center"]

The *Game Setup* dialog contains the following options:

*Game Type*:: NeoRadiant ships with support for several different game engines,
 each of which is contained within a `.game` file. Choose the game type that
 matches the engine you are targeting (Doom 3, Quake 4, Prey, etc.).
...
NeoRadiant interface.

NOTE: It is possible to *Cancel* the *Game Setup* dialog and proceed to the main
window without configuring a game installation, in which case NeoRadiant will
show a warning and ask if you wish to proceed. If you do, NeoRadiant will run
but there will be no available textures, models, entities or other game assets.

=== Interface layout

NeoRadiant uses a flexible layout system based on dockable windows which can be moved into
...
NeoRadiant defaults to a dark colour scheme. Several other colour schemes are available and
...
Colour schemes only affect what is rendered in the 2D and 3D views by NeoRadiant itself.
...
IMPORTANT: The fully lit rendering mode in NeoRadiant is not identical to what
...
NeoRadiant allows you to create brushes directly in the 3D camera view, without
needing to switch to the 2D ortho views.

To create a brush, ensure that nothing is selected, then *Left drag* on any
surface in the 3D view. NeoRadiant will detect the surface plane under the
...
TIP: Like other editors in the Radiant family, NeoRadiant offers a rather
...
NeoRadiant provides six buttons to quickly flip or rotate objects (in 90 degree
...
infinite solid and rooms are carved out), NeoRadiant uses additive geometry:
...
that way. NeoRadiant offers several options for creating multi-sided brushes in
...
NeoRadiant provides a couple of shortcuts. These options can be found on the
...
is useful, so it is retained in NeoRadiant.
|===
...
. Select a brush.
. Activate *Select Edges* with the toolbar button or *E* key. NeoRadiant will
...
NeoRadiant provides several Constructive Solid Geometry (CSG) operations in the
...
Just like with brushes, NeoRadiant offers several default patch shapes beyond
...
3 and NeoRadiant does not permit the creation of a perfect sphere).
...
NeoRadiant provides a *Generate* menu with tools for procedurally creating
...
When NeoRadiant first starts up no textures are loaded and this panel is empty.
...
face. NeoRadiant provides the *Surface Inspector* for this purpose, which can
...
order to assist with this, NeoRadiant provides a *shader clipboard* which
...
NeoRadiant provides certain common functionality to all entities, such as the
...
and patches created inside NeoRadiant, or from an external model file in ASE or
...
view and choose *Create model…*. NeoRadiant will show the model selector
dialog:
...
2D view and choose *Place Player Start here*. NeoRadiant will create the
player start entity at the clicked position.

Since it makes no sense to have more than one player start location, NeoRadiant
...
When you create a model using the <<CreatingModels,model selector>>, NeoRadiant
...
primitives created within NeoRadiant. In either case, the behaviour of
...
continuously. NeoRadiant offers dedicated commands to convert to/from a
...
*TAB* will cause NeoRadiant to select a different primitive contained within
...
NeoRadiant does not include functionality for compiling a map into the form
...
and NeoRadiant to be visible on the screen as separate windows, which you can
...
After the map compilation has failed with a leak, return to NeoRadiant and
...
any available command in NeoRadiant. Type to filter the list using fuzzy matching.
...
NeoRadiant supports importing and opening maps in several formats beyond the
...
Since a fully developed map is large and complex, NeoRadiant provides
...
NeoRadiant allows a number of objects to be grouped together, so that they can
...
NeoRadiant allows a collection of objects to be saved to disk as a *prefab*,
...
prefabs are a NeoRadiant-only feature that have no significance to the engine.
...
NeoRadiant provides a limited ability to export scene geometry into a number of
...
frequently-used map elements — NeoRadiant is not intended to compete with
...
. Select the geometry to be exported. Note that NeoRadiant currently does not
...
objects, NeoRadiant also provides a mechanism for controlling the visibility of
...
In addition to the filters supplied with the mod distribution, NeoRadiant also
...
become more challenging. To assist with this, NeoRadiant provides an *Entity
...
*Export selected as Collision Model...*:: Allows a selected map object (typically
 ...
 After choosing this option, NeoRadiant will display the model chooser dialog,
...
*Reload Declarations / Reload Images / Reload Models / Reload Selected Models*::
 Forces NeoRadiant to re-read the selected category of items from disk,
 refreshing its internal data structures. This is necessary if you have edited
 or re-exported some asset which NeoRadiant is using in the current map and you
...
*Exit*:: Exit NeoRadiant, prompting to save the current map if necessary.
...
*About*:: Display version and build information about NeoRadiant.

*Community Forum*:: Open the community forum in a web browser.

*Script Reference*:: Open the scripting reference documentation.

*User Guide*:: Open the user guide.

== Configuring NeoRadiant

NeoRadiant offers a large number of configurable options which can be used to
...
*Higher selection priority for entities*:: When using the mouse to click on and
 ...
 controls whether NeoRadiant will prefer to select the entity (enabled) or the
 brush (disabled).

==== User Interface settings

*Start on Monitor*:: If you have more than one monitor, you can choose which
monitor NeoRadiant will start on using this combo box.
...
you to choose which language is used for the NeoRadiant interface.

==== Autosave settings

*Enable Autosave*:: Controls whether the autosave feature is active or disabled.

*Autosave Interval*:: Use this slider to choose how often NeoRadiant will make
 an autosave. By default, autosaves are made every 5 minutes.

*Save Snapshots*:: If this option is enabled, NeoRadiant will make each
 ...
 against NeoRadiant or system crashes but not against long-term erroneous
 changes.
...
*Max Snapshot size per map*:: This allows you to control the amount of disk
 ...
 grows above this limit, NeoRadiant will start to delete old snapshots.
...
*Ignore light volume bounds when calculating default rotation pivot location*::
 ...
 when multiple objects are selected. NeoRadiant will place the rotation widget
...
*Undo Queue Size*:: Use this spinbox to control the number of operations which
 are saved onto NeoRadiant's internal undo stack. Larger values provide a
...
*Open last map on startup*:: Enable this option to make NeoRadiant
...
NeoRadiant is designed to work out of the box with a standard Doom 3 or Dark
...
NeoRadiant offers a mechanism to deal with this by allowing certain assets to
...
shaders contained within it will be hidden in NeoRadiant. This means that in order to
...
The following table lists the default keyboard shortcuts in NeoRadiant. These
