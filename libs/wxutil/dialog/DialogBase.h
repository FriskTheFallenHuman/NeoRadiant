...
/**
 * \brief Base class for many NeoRadiant dialogs.
 *
...
public:
    /// Construct and initialise a dialog that is a child of NeoRadiant's main window (if present)
    ...
    // dialog will try to be a child to NeoRadiant's main window)
    DialogBase(const std::string& title, wxWindow* parent);

    // Construct and initialise a named dialog that is a child of NeoRadiant's main window (if present)
    ...
    /**
     * Adjust this window to fit the display NeoRadiant is currently (mainly)
