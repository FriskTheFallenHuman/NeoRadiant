...
namespace
{
	...
	constexpr const char* const INIT_SCRIPT_FILENAME = "init.lua";
	constexpr const char* const LUA_FILE_EXTENSION	 = "lua";
...
{
	_luaModule->addInterface( NamedInterface( name, iface ) );
}
...
{
	_luaModule->executeScriptFile( _scriptPath, filename, setExecuteCommandAttr );
}

ExecutionResultPtr ScriptingSystem::executeString( const std::string& scriptString )
{
	return _luaModule->executeString( scriptString );
}
...
void ScriptingSystem::foreachScriptCommand( const std::function<void( const IScriptCommand& )>& functor )
{
	for( const auto& pair : _commands ) {
		if( pair.first == EXAMPLE_SCRIPT_NAME )
			continue; // skip the example script
...
void ScriptingSystem::executeCommand( const std::string& name )
{
	// Sanity check
	if( !_initialised ) {
		rError() << "Cannot execute script command " << name << ", ScriptingSystem not initialised yet." << std::endl;
		return;
	}
...
void ScriptingSystem::loadCommandScript( const std::string& scriptFilename )
{
	...
	if( result.second ) {
		...
		rError() << "Error in " << scriptFilename << ": Script command " << command->getName() << " has already been registered in " << _commands[command->getName()]->getFilename() << std::endl;
	}
}
...
void ScriptingSystem::initialiseModule( const IApplicationContext& ctx )
{
	...
	addInterface( "GlobalEntityClassManager", std::make_shared<EClassInterface>() );
	...
	addInterface( "ModelSkinCache", std::make_shared<SkinInterface>() );
	addInterface( "SoundManager", std::make_shared<SoundInterface>() );
	addInterface( "DialogManager", std::make_shared<DialogInterface>() );
	...
	GlobalCommandSystem().addCommand( "RunScript", std::bind( &ScriptingSystem::runScriptFile, this, std::placeholders::_1 ), { cmd::ARGTYPE_STRING } );

	GlobalCommandSystem().addCommand( "ReloadScripts", std::bind( &ScriptingSystem::reloadScriptsCmd, this, std::placeholders::_1 ) );

	GlobalCommandSystem().addCommand( "RunScriptCommand", std::bind( &ScriptingSystem::runScriptCommand, this, std::placeholders::_1 ), { cmd::ARGTYPE_STRING } );

	SceneNodeBuffer::Instance().clear();

	// ScriptingSystem is a lazy module, so initialiseModule() is called late enough that
	// we can initialise Lua immediately.
	initialise();
}
